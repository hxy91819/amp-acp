...
For every new ACP session, amp-acp reads Amp's documented `// @amp-agent-mode {"key":"…","label":"…"}` metadata from valid project and system plugin entry files. A single-file plugin must be directly inside its plugin directory; a directory plugin contributes only `index.ts` or, when that is absent, `index.js`. Plugin sources are applied as Amp does: project, system, personal, then workspace, so a higher-precedence plugin entirely replaces a same-named lower-precedence plugin before its metadata is read. No plugin is imported or evaluated merely to build the selector.

Amp has no public non-executing command that proves which Personal or Workspace plugins are active. Configure checked-out active plugin locations with `AMP_ACP_PERSONAL_PLUGIN_PATHS` and `AMP_ACP_WORKSPACE_PLUGIN_PATHS` (platform path-delimited). A downloaded global-plugin cache is deliberately ignored unless its active cache root is explicitly selected with `AMP_ACP_GLOBAL_PLUGIN_CACHE_DIR`; amp-acp then uses only the newest cached revision for each plugin. This configuration boundary prevents old, other-account, or disabled cache entries from being advertised as current modes.
...
Remote discovery reads the effective ordered `dialModes` from Amp's authenticated `getUserInfo` endpoint, the same data used by the Amp CLI. This takes precedence over `AMP_ACP_MODE_KEYS`. The list is cached on disk for **24 hours** by default and shared across projects, ACP sessions, and adapter processes. New or resumed sessions reuse a fresh entry; the next session after expiry refreshes it. No polling or per-prompt lookup is needed. Existing sessions keep their selected mode; a resumed session whose saved mode has left the current cached Dial is rejected rather than switched to another mode.

Set `AMP_ACP_DIAL_CACHE_TTL_SECONDS` to change the lifetime (for example, `604800` for seven days). Set it to `0` to fetch on every new or resumed session and update the shared cache, then remove the override to restore normal caching. To immediately pick up a saved Dial change without changing configuration, delete the adapter's `remote-dial` cache directory and create a new session. A client's cached model picker may also need a refresh.

Cache files live in `$XDG_CACHE_HOME/amp-acp/remote-dial` (default `~/.cache/amp-acp/remote-dial`). Entries are isolated by service URL and a hash of the login credential; changing credentials causes a fresh lookup. Only mode keys and cache timestamps are stored, never credentials or the full account response. Writes are atomic with owner-only file permissions. Concurrent misses in one adapter process share a request; separately started processes can each fetch on a simultaneous cold miss. Missing, damaged, or inaccessible cache files trigger a fresh lookup; an unwritable cache does not prevent a successful session.
...
When a fresh lookup is needed, missing saved Dial settings, authentication errors, timeouts, and invalid responses fail session creation explicitly. Failed requests are not cached, and expired entries are not used as a fallback. A still-fresh entry remains usable while offline. Remote discovery never falls back to the built-in mode list or a manually configured local snapshot. Resetting your remote Dial to Amp's standard default may remove the saved list; select local discovery if you want the default catalog instead.

...
Remote discovery reads the effective ordered `dialModes` from Amp's authenticated `getUserInfo` endpoint, the same data used by the Amp CLI. This takes precedence over `AMP_ACP_MODE_KEYS`. The list is cached on disk for **5 minutes** by default and shared across projects, ACP sessions, and adapter processes. New or resumed sessions reuse a fresh entry; the next session after expiry refreshes it. No polling or per-prompt lookup is needed. Existing sessions keep their selected mode; a resumed session whose saved mode has left the current cached Dial is rejected rather than switched to another mode.

Set `AMP_ACP_DIAL_CACHE_TTL_SECONDS` to change the lifetime (for example, `3600` for one hour). Set it to `0` to fetch on every new or resumed session and update the shared cache, then remove the override to restore normal caching. To immediately pick up a saved Dial change without changing configuration, delete the adapter's `remote-dial` cache directory and create a new session. A client's cached model picker may also need a refresh.

...
Every product change starts on an independent `feature/*` or `fix/*` branch and worktree. That worktree owns implementation, tests, commits, `$autoreview` closeout, and fork publication. Invoke `$autoreview` there after development is complete; the branch is verified for aggregation only after that review reports no accepted/actionable findings. The aggregate receives only verified commits through `git cherry-pick -x`; repair aggregate conflicts in the source worktree and reintroduce a new source commit instead of creating product-only aggregate fixes.

Commit and push every completed, verified change, including documentation and configuration, to the personal fork with normal non-forced pushes; publish the affected source branches and the completed `local/aggregate`. Prepare the verified, committed, and pushed result before requesting deployment confirmation. Once the user confirms deployment for that scope, deploy directly to the current environment and verify that it takes effect; reuse existing confirmation without asking again.
...
| --- | --- | --- | --- |
| `fix/amp-steer-v0.10` | `9344f5be` | `55232e89` | [#61](https://github.com/tao12345666333/amp-acp/issues/61) |
| `fix/amp-persistent-steer-v0.10` | `c47d2c7d` | `03c4f12c` | [#61](https://github.com/tao12345666333/amp-acp/issues/61) |
